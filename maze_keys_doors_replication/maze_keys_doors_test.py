...
Added complexity over simple BFS:
- Expanded state: (row, col, keys_collected)
- Temporal dependencies: key 'a' must be collected BEFORE passing door 'A'
- Planning required: greedy pathfinding is not sufficient
"""
...
def call_claude(prompt: str, model: str = "sonnet") -> Dict[str, Any]:
    """
    Call Claude and return both the result text and full response for analysis.

    Returns: {"result": str, "full_response": str, "usage": dict}
    """
...
def run_nl_condition(maze: List[List[Any]], max_attempts: int = 3, model: str = "sonnet") -> Dict:
...
def run_code_mental_condition(maze: List[List[Any]], max_attempts: int = 3, model: str = "sonnet") -> Dict:
...
def run_code_executed_condition(maze: List[List[Any]], max_attempts: int = 3, model: str = "sonnet") -> Dict:
...
def run_experiment(n_trials: int = 20, model: str = "sonnet", max_attempts: int = 3):
    ...
    print("=" * 70)
    print("KEYS-AND-DOORS MAZE TEST")
    print("=" * 70)
    print(f"\nModel: {model}")
    ...
    # Save results
    filename = f"maze_keys_doors_{model}.json"
